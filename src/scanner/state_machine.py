...
class StateContext:
    ...
    # Agreement guidebook §3 P0-2: percent-distance trigger→price for EXTENDED.
    # Positive means price is ABOVE trigger (extended). None means N/A.
...
def determine_state(match: PatternMatch, ctx: StateContext, prior_state: PatternState | None,
    ...
    # ── Agreement guidebook §3 P0-2: ATR-aware EXTENDED guard ─────────
    # Works for ALL patterns (daily and intraday), not just those with
    # bars_since_breakout. Two independent triggers:
    #   (a) raw % distance — trip if price is > 5 % above trigger
    #   (b) ATR-normalized distance — trip if breakout_distance_atr ≤ -2.5
    # Either one is sufficient. Volatile names will trip (b) first;
    # quiet names will trip (a) first.
    if ctx.extended_pct is not None and ctx.extended_pct >= 0.05:
        return PatternState.EXTENDED
    if ctx.breakout_distance_atr is not None and ctx.breakout_distance_atr <= -2.5:
        return PatternState.EXTENDED
    # Legacy condition retained for symmetry (long-extended, post-breakout drift)
