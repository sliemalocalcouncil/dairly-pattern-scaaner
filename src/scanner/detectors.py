...
def detect_vcp(daily: pd.DataFrame, lookback: int = 90) -> PatternMatch | None:
    """VCP detector — agreement guidebook §3 P0-5.

    Strict thresholds vs original:
      - lookback 130 → 90 (avoid stale bases)
      - base_bars hard ceiling 80 daily bars
      - current price must be within +5 % of trigger (not extended)
      - last contraction must be ≤ 12 % (true tightness)
      - target is computed from the RECENT base (last 3 contractions),
        not from the all-time first pivot
    """
    if len(daily) < 60:
        return None
    ...
    # Near-monotone decreasing (each ≤ 0.95 × prior — design fix C12)
    ...
    if base_bars > 80:
        return None  # base too long; trigger likely stale

    current = float(sub["close"].iloc[-1])
    if current > final_pivot * 1.05:
        return None  # already extended ≥ 5 % above trigger; not a buy zone

    last_contraction = contractions[-1]
    if last_contraction > 0.12:
        return None  # final pivot still too loose; not a true VCP

    # ── Recompute target from the RECENT base (last ≤3 contractions) ───
    recent_pairs = pairs[-3:]
    recent_high = max(h.price for h, _, _ in recent_pairs)
    recent_low = min(l.price for _, l, _ in recent_pairs)
    recent_base_height = recent_high - recent_low
    measured_move_target = float(final_pivot + recent_base_height)
    ...
    return PatternMatch(
        ...
        structure={
            ...
            "neckline_distance_pct": float((final_pivot - current) / current),
            "base_bars": base_bars,
            "recent_base_height": float(recent_base_height),
        },
        geometry_features={
...
def detect_breakout_retest_hold(hourly: pd.DataFrame, daily: pd.DataFrame,
    ...
    for P in pivots_high[-6:]:
        ...
        # 5. Retest must reach down close to P (within 0.25 ATR ABOVE OR 1.5 ATR BELOW)
        # Per agreement guidebook §3 P0-3: retest_low must reach "near or below" the
        # level, not float significantly above it. Floating retests cause negative
        # measured-move targets later. Allowed range:
        #   trigger - 1.5*ATR  ≤  retest_low  ≤  trigger + 0.25*ATR
        if retest_low > P.price + atr_h * 0.25:
            continue
        if retest_low < P.price - atr_h * 1.5:
            continue

        # 6. Retest must not have lost the level (more strict than ATR check above)
        ...
        # 9. Safe target calculation — guard against negative measured move
        # (Per agreement guidebook §3 P0-3 patch 2:)
        retest_depth = max(P.price - retest_low, 0.5 * atr_h)
        target = P.price + retest_depth * 2
        # Belt-and-suspenders: reject if target somehow ends up below trigger
        if target <= P.price:
            continue
