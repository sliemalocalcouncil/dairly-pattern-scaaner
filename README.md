...
## v2.3.0 (현재) — Alert Quality Overhaul (가이드북 §3 P0/P1 일괄 적용)

리뷰팀 + 35건 실제 데이터 분석 결과를 반영해 **알림 품질 게이트** 전면 강화. 가이드북 합의서에 따라 detector 추가 없이 차단 로직만 강화한 PR. 35건 retroactive 검증에서 **23개 entry alert 중 20개 차단 (87%)** + 12개 risk event 100% 보존 확인.

| ID | 변경 | 효과 |
|----|------|------|
| **P0-1** | `alert_quality_gate()` — Telegram 발송 직전 5종 hard reject (target<trigger, degenerate stop, wide stop, breached stop, extended price) | VNET 같은 깨진 신호 자동 차단 |
| **P0-2** | EXTENDED state 강화 — daily 패턴도 `price > trigger × 1.05` 면 EXTENDED, ATR 기준 -2.5 도 추가 트리거 | INTC/AVGO/MU 류 추격 알림 차단 |
| **P0-3** | `breakout_retest_hold` detector 수정 — retest_low 가 `trigger ± ATR` 안에 있어야 인정, target 음수 방지 | target<trigger 신호 원천 차단 |
| **P0-4** | BREAKOUT_CONFIRMED 도 `min_alert_score` 적용 (FAILED/INVALIDATED 만 면제) | 점수 40~60 BREAKOUT 알림 차단 |
| **P0-5** | VCP detector strict — lookback 130→90, base_bars≤80, current≤trigger×1.05, last_contraction≤12% | 오래된 base 추격 차단 |
| **P0-6** | `apply_vetoes()` 에 stop_pct hard cap 추가 (>15% → 55, >25% → 40), price_extended cap, target<trigger cap | risk weight 5%로 못 잡던 신호 차단 |
| **P1-1** | Trend floor veto 차등화 — 일반 trend-sensitive T<30 cap, retest_hold T<50 cap, reversal T<50 cap | 추세 약한 종목의 continuation 알림 차단 |
| **P1-2** | 패턴별 `min_alert_score` 2단계 분리 — reversal 패턴 (Double Bottom, IHS) 은 +8 점 bump | 반전 패턴 false positive 감소 |

**테스트**: 4개 기존 suite + 2개 신규 suite (P0/P1 unit tests 27건 + 35건 retroactive verification 6 acceptance criteria) = **53 test cases all green**.
